...
const lockedPreview=`\n<section class="marketing-section" id="gym-preview"><div class="marketing-inner"><div class="section-kicker">Practice Gym · email access</div><h2>The 29-question Gym is locked until signup.</h2><p class="section-lead">Submit your email once to unlock the Micro1/Zara Prep Guide and full Practice Gym on this browser for 30 days. The question bank, guided answers, mock sessions, delivery mechanics, and glossary are not included in the public page source.</p><div class="proof-stats"><div class="proof-stat"><b>29</b><span>practice questions</span></div><div class="proof-stat"><b>35</b><span>adaptive follow-ups</span></div><div class="proof-stat"><b>6 / 8</b><span>mock formats</span></div><div class="proof-stat"><b>30d</b><span>browser access</span></div></div><div class="hero-actions" style="margin-top:24px"><a class="marketing-primary" href="#access">Unlock Guide + Gym</a></div></div></section>`;

const leadScript=`\n<script>\n(()=>{\n const form=document.getElementById('leadForm'),email=document.getElementById('leadEmail'),website=document.getElementById('leadWebsite'),success=document.getElementById('leadSuccess'),error=document.getElementById('leadError'),guideLink=document.getElementById('guideLink'),gymLink=document.getElementById('gymLink');\n if(!form||!email||!success||!error)return;\n const button=form.querySelector('button[type="submit"]');\n const activate=(guide,gym)=>{guideLink.href=guide||'/api/guide';gymLink.href=gym||'/api/gym#gym';success.classList.add('show');button.textContent='Access active';email.disabled=true;if(website)website.disabled=true;};\n fetch('/api/access',{headers:{'Accept':'application/json'}}).then(r=>r.json()).then(d=>{if(d&&d.authorized)activate(d.guide,d.gym);}).catch(()=>{});\n form.addEventListener('submit',async e=>{\n  e.preventDefault();error.classList.remove('show');success.classList.remove('show');\n  if(!email.checkValidity()){email.reportValidity();return;}\n  const original=button.textContent;button.disabled=true;button.textContent='Submitting…';\n  try{\n   const res=await fetch('/api/leads',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({email:email.value.trim(),website:website?website.value:'',source:'micro1-zara-pack'})});\n   let data={};try{data=await res.json()}catch(_){}\n   if(!res.ok||!data.ok||!data.guide||!data.gym)throw new Error(data.error||'Your email could not be saved.');\n   if(data.offer){window.location.assign(data.offer);return;}\n   activate(data.guide,data.gym);\n  }catch(err){button.disabled=false;button.textContent=original;error.textContent='We could not confirm your signup, so access was not unlocked. Please try again.';error.classList.add('show');}\n });\n})();\n</script>\n</body></html>`;
...
if(!publicHtml.includes("fetch('/api/access'")) throw new Error('Returning-session check missing');
if(!publicHtml.includes('30 days')) throw new Error('30-day access messaging missing');
...
console.log('Built public landing + protected Guide/Gym v5; optional $29 post-signup tripwire supported');
